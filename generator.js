// ─── Content Generator ─────────────────────────────────────
// Takes a syllabus topic and uses the LLM to generate lesson content.
// Generated content is cached in the database for reuse.
...
async function generateVocabulary(course, module, topicIndex) {
  ...
  const prompt = buildVocabPrompt(course, module, topic);
  ...
  db.saveContent(cacheKey, lesson);
  return lesson;
}
...
async function generateQuiz(course, module, variant = 0) {
  ...
  const prompt = buildQuizPrompt(course, module, variant);
  const content = await llm.chatJSON([
    { role: 'system', content: SYSTEM_PROMPT },
    { role: 'user',   content: prompt }
  ]);
...
async function generateSpeaking(course, module, variant = 0) {
  ...
  const prompt = buildSpeakingPrompt(course, module, variant);
  const content = await llm.chatJSON([
    { role: 'system', content: SYSTEM_PROMPT },
    { role: 'user',   content: prompt }
  ]);
...
// ═══════════════════════════════════════════════════════════════
// Get the lesson list for a module (topics → vocab lessons + speaking + quiz)
// ═══════════════════════════════════════════════════════════════
function getLessonList(course, module) {
  const lessons = [];
  const isLanguage = course.targetLanguage !== course.nativeLanguage;

  // One vocabulary lesson per topic
  ...
  // One speaking exercise per module (only for language courses)
  if (isLanguage) {
    const speakCacheKey = `speak:${course.id}:${module.id}:0`;
    const speakCached = db.getContent(speakCacheKey);
    lessons.push({
  ...
  const quizCached = db.getContent(quizCacheKey);
  lessons.push({
    id: `${module.id}-quiz-0`,
...
function buildVocabPrompt(course, module, topic) {
  const isLanguage = course.targetLanguage !== course.nativeLanguage;
  const count = '6 to 8';
...
- The "examplePt" must sound natural in European Portuguese, not a word-by-word translation`;
  ...
  // Non-language subject (math, science, etc.)
...
function buildQuizPrompt(course, module, variant) {
  const isLanguage = course.targetLanguage !== course.nativeLanguage;
  const count = '5 to 7';
...
// ───────────────────────────────────────────────────────────────
function buildSpeakingPrompt(course, module, variant) {
...
module.exports = { generateVocabulary, generateQuiz, generateSpeaking, getLessonList, getLesson };
